...
| [Battle Art](https://github.com/absol89/DramaticShapeVoxelMod) | `BATTLE_ART_VOXEL_FORK` 1.8.8; commit `e82a3bc2ea45858f612ab55b482412a231bce7e1` | Its merged `exports.battleStage` API v1 is the stable read-only compatibility surface used by SBFX 2.1.2 and later. | [PR #10](https://github.com/absol89/DramaticShapeVoxelMod/pull/10) registers its voxel-map arena through Battle Presentation API v1 and calls `drawActors(world)` so models and all other SBFX features remain independently selectable. |
...
| [Potato Voxel](https://github.com/ShaneMcGovernIE/potato_voxel) | `potato_voxel` 1.5.2; commit `0e4cd1b417a19cd082370b76180ee0238c188717` | The read-only Shape-family bridge detects its staged battle and prevents duplicate arena/model/camera ownership. | [PR #41](https://github.com/ShaneMcGovernIE/potato_voxel/pull/41) registers its performance-scaled voxel-map arena through Battle Presentation API v1 and calls `drawActors(world)`, emphasizing compatibility and mix-and-match rather than load order. It rejects the legacy disc arena. |
