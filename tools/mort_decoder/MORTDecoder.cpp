	...
	while (true)
	{
		...
		//for (int soundIndex = 0; soundIndex < 2; soundIndex++)
		{
			...
			if (T6 != 2) // One playing
			{
				...
				if (pcmSamples.size() >= expectedSamples)
				{
					pcmSamples.resize(expectedSamples);
					break;
				}
...
void CMORTDecoder::ClearBuffer_Function80057FD0(unsigned long A0Param, unsigned long A1Param)
{
	...
	for (unsigned long x = 0; x < A1; x++)
	{
		...
		{
			// The reference GUI displayed a diagnostic here. The headless helper
			// continues through the same decoder state without UI dependencies.
		}
	}
}
